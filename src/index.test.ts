...
import { parseLine, detectFormat, extractToolCalls, buildSession, recordFromText } from './recorder.js';
import { diffSessions, computeSimilarity, detectRegressions } from './differ.js';
import { init, createSnapshot, listSnapshots, loadSnapshot, deleteSnapshot, findSnapshotsByTask } from './snapshots.js';
import { AgentSession, SessionEvent, ToolCall } from './types.js';
...
await (async function testDetectFormat() {
  assert.strictEqual(detectFormat(JSON.stringify({ type: 'tool_call', tool: 'read', timestamp: '' })), 'generic');
  assert.strictEqual(detectFormat(JSON.stringify({ role: 'assistant', content: [] })), 'claude-code');
  assert.strictEqual(detectFormat(JSON.stringify({ type: 'response', model: 'gpt-4' })), 'codex');
  assert.strictEqual(detectFormat('not json'), 'generic');
...
await (async function testBuildSession() {
  ...
  const session = buildSession(events, 'test task', 'test-agent', 'generic');
...
await (async function testRecordFromText() {
  ...
  const session = recordFromText(jsonl, 'test', 'agent');
...
await (async function testDiffIdenticalSessions() {
  ...
  const diff = diffSessions(session, session);
...
await (async function testDiffRemovedToolCalls() {
  ...
  const diff = diffSessions(base, curr);
...
await (async function testComputeSimilarity() {
  ...
  const sim = computeSimilarity(s1, s2);
...
await (async function testComputeSimilarityEmpty() {
  ...
  assert.strictEqual(computeSimilarity(s1, s2), 1.0);
...
await (async function testCreateAndListSnapshots() {
  ...
  init(tmpDir);
  const session = mockSession();
  const meta = createSnapshot(tmpDir, session, ['baseline']);
...
await (async function testLoadSnapshot() {
  ...
  init(tmpDir);
  const session = mockSession();
  createSnapshot(tmpDir, session);

  const loaded = loadSnapshot(tmpDir, session.id);
  assert.ok(loaded);
...
await (async function testDeleteSnapshot() {
  ...
  assert.strictEqual(deleteSnapshot(tmpDir, session.id), true);
  assert.strictEqual(deleteSnapshot(tmpDir, session.id), false);
  assert.strictEqual(listSnapshots(tmpDir).length, 0);
  fs.rmSync(tmpDir, { recursive: true });
...
await (async function testFindByTask() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agentsnap-'));
  init(tmpDir);
  createSnapshot(tmpDir, mockSession({ task: 'add auth middleware' }));
  createSnapshot(tmpDir, mockSession({ id: 'snap_test_002', task: 'fix login bug' }));
  createSnapshot(tmpDir, mockSession({ id: 'snap_test_003', task: 'add auth middleware' }));

  const results = findSnapshotsByTask(tmpDir, 'auth');
